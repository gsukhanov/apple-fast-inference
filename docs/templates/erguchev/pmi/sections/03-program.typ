...
3. Реализация слоев нейронной сети
    - Выполнение сверточных операций (Conv2D) с использованием различных бэкендов (CPU/NEON im2col, AMX).
...
2. Результаты верификации:
    - Значения численной погрешности при сравнении CPU/NEON- и AMX-реализаций.
...
#figure(
    table(
        ...
        rows: (10mm, auto),
        align: left + horizon,
        table.header([Этап API], [Проверяемое действие]),
        [Создание тензоров], [Инициализация входных матриц через описание формы, типа данных и выбранного режима исполнения.],
        [Вызов операции], [Запуск матричного умножения через общий интерфейс, скрывающий различия между CPU/NEON- и AMX-путями.],
        [Проверка результата], [Сравнение выходного тензора с эталонным CPU-результатом в пределах допустимой численной погрешности.],
        [Диагностика], [Проверка сообщения об ошибке при несовместимости размерностей входных матриц.],
    ),
    caption: [Структура проверки C++ API для создания тензора и вызова матричного умножения],
)
