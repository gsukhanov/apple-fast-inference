...
#paragraph[
  Выходные данные бенчмарков и тестов используются в двух целях. Во-первых, они подтверждают функциональную корректность: тесты LeNet должны выдавать ожидаемые классы для подготовленных изображений. Во-вторых, они формируют основу для оценки производительности: результаты замеров используются для построения графика ускорения AMX-исполнения относительно CPU/NEON, приведенного в приложении 3.
]
