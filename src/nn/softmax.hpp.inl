...
typename Softmax<_DType, _Device>::tensor_t Softmax<_DType, _Device>::forward(
    ...
    tensor_t output(shape, scalar_t{});
    ...
    while (true) {
        scalar_t max_value = std::numeric_limits<scalar_t>::lowest();
        ...
        for (std::int64_t k = 0; k < axis_size; ++k) {
            ...
            output.at(idx) = static_cast<scalar_t>(exp_val);
            sum += exp_val;
        }
        ...
        for (std::int64_t k = 0; k < axis_size; ++k) {
            idx[axis_idx] = k;
            output.at(idx) = static_cast<scalar_t>(
